...
1) Модель данных `MailingRecipient(models.Model)` - представляет "Получателя рассылки" в сервисе управления рассылками.
   - ***Поля модели:***
...
3) Модель данных `Mailing(models.Model)` - представляет "Рассылку" в сервисе управления рассылками.
   - ***Поля модели:***
     ...
     - Получатели для рассылки (mailing_recipients) - "многие ко многим", связь с моделью "Получатель" (*models.ManyToManyField(to=MailingRecipient)*).
