...
[10. Вспомогательные функции (managers.py)](#title10) / 
[11. Установка проекта](#title11) / 
[12. Получение ключей .env](#title12) / 
[13. Описание файла .flake8](#title13) / 
[14. Описание файла mypy.ini](#title14) / 
[15. Описание директории Data](#title15) / 
...
1) Класс-контроллер `RecipientListView(LoginRequiredMixin, generic.ListView)` - представление для отображения списка *Получателей* рассылки.
   ...
   - ***Кэширование***:
     - используется декоратор для создания кеша для всей страницы с параметром *key_prefix*, который потом нужен для сброса кэша страницы при редактировании/изменении данных какого либо объекта (получателя): `@method_decorator(cache_page(60 * 15, key_prefix="recipients_list"), name="dispatch"`.
...
3) Класс-контроллер `RecipientUpdateView(LoginRequiredMixin, generic.UpdateView)` - представление для редактирования существующего *Получателя* рассылки.
   - ***Кастомизация контроллера***:
     ...
     - `def form_valid(self, form)` - метод для:
       ...
       - Сброса кэша при обновлении данных какого-либо *Получателя* из списка.
...
1) Класс-контроллер `MessageListView(LoginRequiredMixin, generic.ListView)` - представление для отображения списка *Сообщений* для рассылок.
   ...
   - ***Кэширование***:
     - используется декоратор для создания кеша для всей страницы: `@method_decorator(cache_page(60 * 15), name="dispatch"`.
...
5) Класс-контроллер `SendMailingView(LoginRequiredMixin, generic.View)` - представление для запуска выбранной пользователем *Рассылки* вручную через интерфейс и фиксации *Попыток рассылок* по каждому *Получателю* из рассылки.
   - ***Кастомизация контроллера***:
     - `def post(self, request, pk)` - метод запускает сервисную функцию ***send_mailing()*** из *services.py*, которая:
...
6) Класс-контроллер `StopMailingView(LoginRequiredMixin, generic.View)` - представление для остановки выбранной пользователем *Рассылки* вручную через интерфейс и фиксации остановки *Попыток рассылок* по каждому *Получателю* из рассылки, которые еще не были отправлены.
   - ***Кастомизация контроллера***:
     - `def post(self, request, pk)` - метод для инициирования остановки рассылки:
        - Завершает рассылку.
...
# <a id="title10">10. Вспомогательные функции (managers.py)</a>
...
# <a id="title13">13. Описание файла .flake8</a> 
...
# <a id="title14">14. Описание файла mypy.ini</a> 
...
# <a id="title15">15. Описание директории data/</a> 
